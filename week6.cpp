...
{
    int register_read = 0;
    int register_written = 0;
    
    for(int i = 0; i < Pipe.OpCount; i++) 
    {
        if(Pipe.IsStageRR(i,c))
            {
                register_read = Pipe.GetRegNum(i,c);
                for(int j = i; j< Pipe.OpCount+i; j++)
                {
                    if(Pipe.IsStageWB(j,c) && Pipe.GetRegNum(j,c) == register_read)
                    {
                        Pipe.InsertStall(j,c);
                    }
                }
            }
        ...
        if(Pipe.IsStageWB(i,c))
            {
                register_written = Pipe.GetRegNum(i,c);
                for(int j = i; j< Pipe.OpCount+i; j++)
                {
                    if(Pipe.IsStageRR(j,c) && Pipe.GetRegNum(j,c) == register_written)
                    {
                        Pipe.InsertStall(j,c);
                    }
                }
                ...
                for(int j = i; j< Pipe.OpCount+i; j++)
                {
                    if(Pipe.IsStageWB(j,c) && Pipe.GetRegNum(j,c) == register_written)
                    {
                        Pipe.SwapOps(j,c);
                    }
                }
